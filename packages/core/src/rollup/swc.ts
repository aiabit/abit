...
export function swc(options: PluginOptions = {}): RollupPlugin {
  ...
  return {
    ...
    async resolveId(importee, importer) {
      ...
      if (importer && importee[0] === '.') {
        ...
        if (
          ...
          (await fs.promises.stat(resolved)).isDirectory()
        ) {
    ...
    async transform(code: string, id: string) {
      ...
      const tsconfigOptions =
        options.tsconfig === false ? {} : getOptions(dirname(id), options.tsconfig);
      ...
      const useReact17NewTransform =
        tsconfigOptions.jsx === 'react-jsx' || tsconfigOptions.jsx === 'react-jsxdev';
